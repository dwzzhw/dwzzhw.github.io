...
四、持续改进
----------------------------------------------------------
前面详细讨论了自适应停等限速方案的原理及实现细节，过程中也提到了基于分片的限速方案。相对于分片方案来说，停等方案在首个大脉冲之后的网络曲线要平滑的多，且整个过程只需一个网络请求，相对于分片方案的每个分片一个请求，效率要明显提高，且不依赖于服务端range字段的支持，对是否长链接也没有要求。但是，很可惜，美中不足的是首个大脉冲太不美观。
那么有办法消除或减小这个丑陋的首脉冲吗？答案是有的。
前面的讨论中提到过，这个脉冲就是网络数据自发填满TCP接收窗口的过程，脉冲的大小取决于接收窗口的大小，而接收窗口的值与socket实现中传递进去的接收buffer的大小是正相关的，具体可以参考Socket实现中的SO_RCVBUF字段。
那么解决这个首脉冲的一个办法就是通过Socket的setReceiveBufferSize方法来减小接收窗口的大小，就可以按需调整首脉冲的高度了。
详细的实现过程就依赖于具体实现了。如果代码中可以直接拿到下载所用socket的引用，那么可以直接调用上述方法进行修改。以我们的下载模块为例，我们使用了OKHTTP的实现，在初始化时，OKHTTP提供了setSocketFactory这个方法，通过这个方法可以干预socket的创建，添加我们感兴趣的参数，比如减小socket的receiveBuffer，通过修改这个接收buffer去降低首脉冲，改进后的效果见下图：
...
与图3-2相比，改进后的首脉冲已经显著降低，可见通过修改Socket的接收buffer的方式是可行的。
